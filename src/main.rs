...
use eframe::{
    egui::{
        self, ComboBox, Label, Rect, Sense, SidePanel, Slider, TopBottomPanel, ViewportBuilder,
    },
    epaint::{Pos2, Vec2},
};
use simulator::{function::Function, simulator::Simulator, Circuit};
...
impl CircuitBuilder {
    fn menu_bar(&mut self, ctx: &egui::Context) {
        self.occupied_sides.top = TopBottomPanel::top("menu_bar")
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
    ...
    fn explorer(&mut self, ctx: &egui::Context) {
        self.occupied_sides.left = SidePanel::left("explorer")
            .show(ctx, |ui| {
                if let Some(circuit) = &self.circuit {
                    ui.label("Circuit");
                    ui.separator();
                    ...
                    ui.collapsing("Inputs", |ui| {
                        for i in 0..circuit.all_inputs().len() {
                            ...
                            if ui
                                .add(
                                    Label::new(format!("{i}: value index: {value_index}"))
                                        .sense(Sense::click()),
                                )
                                .clicked()
                            {
                    ...
                    ui.collapsing("Components", |ui| {
                        for i in 0..circuit.all_components().len() {
                            ...
                            ui.collapsing(format!("{i}: {function}"), |ui| {
                                if ui
                                    .add(Label::new(format!("Select")).sense(Sense::click()))
                                    .clicked()
                                {
                ...
                if self.circuit.is_some() && self.simulator.is_none() {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        if self.circuit.as_ref().unwrap().value_list_len() > 0 {
                            ...
                            if ui.button("Component").clicked() {
                                self.adding_element =
                                    AddingElement::Component(AddComponentData::default());
                            }
                        }
                    });
                    ...
                    match &mut self.adding_element {
                        ...
                        AddingElement::Output(index) => {
                            ...
                            let mut value = *index;
                            let value_range =
                                0..=self.circuit.as_ref().unwrap().value_list_len() - 1;
                        ...
                        AddingElement::Component(component_data) => {
                            ...
                            let current_discriminatn =
                                std::mem::discriminant(&component_data.function);
                            let mut value = options
                                .iter()
                                .position(|value| {
                                    current_discriminatn == std::mem::discriminant(value)
                                })
                                .unwrap();

                            ComboBox::from_label("Choose function").show_index(
                                ui,
                                &mut value,
                                options.len(),
                                |i| format!("{}", options[i]),
                            ...
                            if current_discriminatn != std::mem::discriminant(&options[value]) {
                                ...
                                if component_data.input_value_indices.len()
                                    != component_data.function.input_value_count()
                                {
                                    component_data
                                        .input_value_indices
                                        .resize(component_data.function.input_value_count(), 0);
                                }
                            }
                            ...
                            for i in 0..component_data.input_value_indices.len() {
                                ui.add(Slider::new(
                                    &mut component_data.input_value_indices[i],
                                    0..=self.circuit.as_ref().unwrap().value_list_len() - 1,
                                ));
                            }

                            if ui.button("Confirm").clicked() {
                                self.circuit.as_mut().unwrap().add_component(
                                    component_data.function.clone(),
                                    component_data.input_value_indices.clone(),
                                );
                                self.adding_element = AddingElement::None;
                            }
                        }
    ...
    fn inspector(&mut self, ctx: &egui::Context) {
        self.occupied_sides.right = SidePanel::right("inspector")
            .show(ctx, |ui| {
                if let Some(circuit) = &self.circuit {
